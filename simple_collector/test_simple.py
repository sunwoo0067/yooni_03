...
def test_collectors():
    ...
    try:
        ...
        if auth_result:
            ...
            print(f"   ✓ 총 {product_count}개 상품 수집 완료")
        
    except Exception as e:
        print(f"   ✗ 젠트레이드 테스트 실패: {e}")
        ...
        if auth_result:
            ...
            for product in collector.collect_products():
                ...
                if product_count <= 3:
                    print(f"     └ 상품: {product.product_code} - {product.product_info.get('product_name', 'N/A')}")
            print(f"   ✓ 총 {product_count}개 상품 수집 완료")
            print(f"   ✓ 캐시된 코드: {len(collector._cached_codes)}개")
        
    except Exception as e:
        print(f"   ✗ 오너클랜 테스트 실패: {e}")
            ...
            try:
                ...
                for product_data in collector.collect_products():
                    ...
                    if existing:
                        ...
                        print(f"   ✓ 상품 업데이트: {product_data.product_code}")
                    else:
                        # 신규 저장
                        new_product = Product(
                        ...
                        db.add(new_product)
                        print(f"   ✓ 신규 상품 저장: {product_data.product_code}")
                ...
                print(f"   ✓ 총 {saved_count}개 상품 저장 완료")
                print(f"   ✓ 데이터베이스 총 상품: {total_products}개")
                print(f"   ✓ 젠트레이드 상품: {zentrade_products}개")
...
def main():
    ...
    if test_database():
        ...
        print("✗ 데이터베이스 테스트 실패로 전체 테스트 중단")
